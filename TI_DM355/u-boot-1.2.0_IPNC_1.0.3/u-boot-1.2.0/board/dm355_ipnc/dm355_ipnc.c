...
int board_init (void)
{
        ...
        gd->bd->bi_arch_number = MACH_DM350_IPNC;
	      ...
	      *((volatile unsigned int *) GIO_SET_DATA23) |= (1<<0); // output Hi
			...
			//shcho 2014-02-25 Booting LED ON
		#define PINMUX3     0x01C4000C
        /* Enable GIO23 Set to GPIO */ // Schematic 오류
        *(volatile unsigned int *)PINMUX3 &=  ~(0x00000080);
	      	/* set GIO27 output	*/
	      *((volatile unsigned int *) GIO_DIR01) &= ~(0x00800000);  
	      *((volatile unsigned int *) GIO_SET_DATA01) |= (0x00800000); // output Hi
