	...
	do {
		...
		if (wCsrVal & MGC_M_TXCSR_P_SENTSTALL) {
			...
			if ((pRequest) && (pEnd->busy)) {
				musb_g_giveback(pEnd, pRequest, -EPIPE);
				printk("[musb_g_tx] pRequest NULL?????????");
			}
			break;
		}
			      ...
			      const struct usb_endpoint_descriptor *desc)
{
	...
	if (desc->bEndpointAddress & USB_DIR_IN) {
		...
		//if (MGC_ReadCsr16(pBase, MGC_O_HDRC_TXCSR, bEnd)
		//		& MGC_M_TXCSR_FIFONOTEMPTY)
...
int musb_gadget_set_halt(struct usb_ep *ep, int value)
{
	...
	if (pEnd->is_in) {
		...
		if (value) {
			...
			wCsr &=
			    ~(MGC_M_TXCSR_P_SENDSTALL | MGC_M_TXCSR_P_SENTSTALL | MGC_M_TXCSR_P_UNDERRUN);
		}
		...
		if (value) {
			...
			wCsr &= ~(MGC_M_RXCSR_P_SENDSTALL | MGC_M_RXCSR_P_SENTSTALL);
		}
...
void musb_g_disconnect(struct musb *pThis)
{
	...
	if(musb_readb(pThis->pRegs, MGC_O_HDRC_DEVCTL)==0x88){
