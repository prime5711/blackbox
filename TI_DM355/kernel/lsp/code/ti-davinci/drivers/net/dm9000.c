...
static unsigned char dm9000_eth_string[20] = "deadbeef";
...
typedef struct board_info {
	...
	u8 chip_revision;    /*Appro added for dm9000b */
	u8 phy_addr;
...
static unsigned char dm9000_str2hexnum(unsigned char c)
{
	if (c >= '0' && c <= '9')
		return c - '0';
	if (c >= 'a' && c <= 'f')
...
static void dm9000_str2ethaddr(unsigned char *ea, unsigned char *str)
{
	int i;
	unsigned char num;
	...
	for (i = 0; i < 6; i++) {
		...
		num = dm9000_str2hexnum(*str) << 4;
		++str;
		num |= (dm9000_str2hexnum(*str));
		++str;
		ea[i] = num;
	}
}
...
dm9000_probe(struct device *dev)
{
   ...
   db->chip_revision = ior(db, DM9000_CHIPR);
   printk("%s: read  chip_revision 0x%08x\n", CARDNAME, db->chip_revision);
  ...
  ndev->do_ioctl = &dmfe_do_ioctl;
	...
	if (strcmp(dm9000_eth_string, "deadbeef") == 0) {
		/* Read SROM content */
		for (i = 0; i < 64; i++)
		...
		/* lets get it from environment variable "ethaddr" */
		char* tmp;
		printk("Getting MAC from env var 'ethaddr'..\n");
		tmp = &dm9000_eth_string[0];
		dm9000_str2ethaddr(ndev->dev_addr, tmp);
	}
...
static int dm9000_eth_setup(char *str)
{
	/* The first char passed from the bootloader is '=', so ignore it */
	strcpy(&dm9000_eth_string[0], &str[1]);

	return (1);
}

__setup("eth", dm9000_eth_setup);
