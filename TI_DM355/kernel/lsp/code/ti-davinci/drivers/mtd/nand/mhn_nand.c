 ...
 *
 *  Overview:
 *   This is a device driver for Monahans NAND Flash Controller
 *
...
#include <asm/delay.h>
#include <asm/dma.h>
#include <asm/arch-pxa/cpu-freq-voltage-mhn.h>
...
enum {
	...
	DFC_DATA_SIZE_2112 = 2112,	/* R/W enabling spare area, but no ecc */
...
{
	offset <<= 2;
	return readl(context->membase + offset);
}
...
static void dfc_set_timing(struct dfc_context *context, struct dfc_flash_timing *t)
{
	...
	r0 = (timing.tCH << DFC_TIMING_tCH) |
	    ...
	    (timing.tRH << DFC_TIMING_tRH) | (timing.tRP << DFC_TIMING_tRP);
...
static static unsigned int get_delta(unsigned int start)
{
...
/******************************************************************************
  ...
  Returns:
	None
*******************************************************************************/
static void dfc_write_fifo_partial(struct dfc_context *context, uint8_t * buffer,
			    int nbytes, int data_size)
{
	uint32_t i = 0;
...
static void dfc_read_fifo(struct dfc_context *context, uint8_t * buffer, int nbytes)
{
	uint32_t i = 0;
...
/******************************************************************************
  ...
  Returns:
	None
*******************************************************************************/
static void dfc_write_fifo(struct dfc_context *context, uint8_t * buffer, int nbytes)
{
	uint32_t i = 0;
	uint32_t bytes_multi = (nbytes & 0xFFFFFFFC);
	uint32_t bytes_remain = nbytes & 0x03;
...
/******************************************************************************
  ...
  Description:
	This function reads bad block address in units of block starting from 0 if
	bad block is detected. It takes into the account if the operation is for
	CS0 or CS1  depending on settings of chip_select parameter of DFC Mode
	structure.
  Input Parameters:
	context
	  Pointer to DFC context structure
  Output Parameters:
	pBadBlockAddr
	  Used to retrieve bad block address back to caller if bad block is detected
  Returns:
	None
*******************************************************************************/
...
/******************************************************************************
	...
	Description:
	  This function does entire DFC initialization according to the NAND flash
	  type currently used with platform, including setting MFP, set flash timing,
	  set DFC mode, configuring specified flash parameters in DFC, clear ECC logic
	  and page count register.
  Input Parameters:
	  context	--	Pointer to DFC context structure
  Output Parameters:
    None
  Returns:
    ...
    -EINVAL
      ...
      block
******************************************************************************/
extern void enable_dfc_pins(void);
...
/******************************************************************************
  ...
  Description:
	It reset the flash. The function can be called at any time when the device
	is in Busy state during random read/program/erase mode and reset operation
	will abort all these operations. After reset operation the device is ready
...
static int dfc_readid(struct dfc_context *context, uint32_t * id)
{
	...
	/* Send command */
	status =
	    dfc_send_cmd(context, (uint16_t) flash_info->read_id, 0xFFFFFFFF,
			 0);
	if (status) {
		ClearCMDBuf();
		return status;
...
#ifdef CONFIG_MTD_NAND_MONAHANS_DMA
	.enable_dma =	1,			/* enable DMA */
#endif
	.enable_ecc =	1,			/* enable ECC */
	.enable_spare =	1,			/* enable SPARE */
};
 ...
 */
static struct mtd_info *monahans_mtd;
...
static struct mtd_partition partition_info[] = {
	...
	{
	 .name = "Filesystem",
	 .size = 0x01000000,		/* only mount 16M fs */
	 .offset = 0x00240000,
	 },
	{
	 .name = "MassStorage",
...
static void print_buf(char *buf, int num)
{
	...
	while (i < num) {
		printk
		    (KERN_ERR "0x%08x: %02x %02x %02x %02x %02x %02x %02x %02x "
...
static void dump_info(struct monahans_dfc_info *info)
{
	if (!info)
		return;
...
#endif
static void enable_hw_ecc(struct dfc_context *context, int enable)
{
	unsigned long ndcr;
...
static void monahans_df_write_buf(struct mtd_info *mtd, const u_char * buf,
				  int len)
{
	struct monahans_dfc_info *info = (struct monahans_dfc_info *)
	    (((struct nand_chip *)(mtd->priv))->priv);
...
static int monahans_df_verify_buf(struct mtd_info *mtd, const u_char * buf,
				  int len)
{
	return 0;
}
				     ...
				     struct pt_regs *regs)
{
	...
	pr_debug
	   ("cmd dma interrupt, channel:%d, DCSR:0x%08x\n", channel, dcsr);
	if (dcsr & DCSR_BUSERR) {
				      ...
				      struct pt_regs *regs)
{
	...
	pr_debug
	   ("data dma interrupt, channel:%d, DCSR:0x%08x\n", channel, dcsr);
	if (dcsr & DCSR_BUSERR) {
		info->retcode = ERR_DMABUSERR;
		complete(&info->cmd_complete);
	}
			    ...
			    unsigned int event)
{
	...
	struct monahans_dfc_info *info =
	    (struct monahans_dfc_info *)(((struct nand_chip *)(mtd->priv))->
					 priv);
	struct dfc_context *context = info->context;
	int status;
	int ret;
	...
	pr_debug
	   ("ready send command, cmd:0x%x, at address:0x%x, num_pages:%d, wait event:0x%x\n",
	...
	pr_debug
	   ("command return, cmd:0x%x, retcode:%d\n", info->cmd,
	    info->retcode);
	return 0;
}
				...
				int column, int page_addr)
{
	...
	struct dfc_context *context = info->context;
	struct dfc_flash_info *flash_info = info->context->flash_info;
	int ret, pages_shift;
	int status;
#ifndef CONFIG_MTD_NAND_MONAHANS_DMA
	int datasize;
	...
	pr_debug
	   ("command:0x%x at address:0x%x, column:0x%x\n", command, page_addr,
	    column);
	...
	case NAND_CMD_PAGEPROG:
		...
		if (info->cmd != NAND_CMD_SEQIN) {
			...
			info->retcode = ERR_SENDCMD;
			printk(KERN_ERR
			       "Monahans NAND device: No NAND_CMD_SEQIN executed before.\n");
			enable_hw_ecc(context, 1);
			break;
		}
		info->cmd = command;
		...
		ret =
		    wait_for_completion_timeout(&info->cmd_complete,
						2 * HZ / 100);
		if (!ret) {
	...
	case NAND_CMD_ERASE1:
		...
		if (info->context->dfc_mode->chip_select)
			ret =
			    dfc_send_command(mtd, flash_info->erase, info->addr,
					     0, NDSR_CS1_BBD | NDSR_CS1_CMDD);
		else
			ret =
			    dfc_send_command(mtd, flash_info->erase, info->addr,
					     0, NDSR_CS0_BBD | NDSR_CS0_CMDD);
		if (info->retcode == ERR_BBERR) {
			mtd->block_markbad(mtd, info->addr);
		}
	...
	case NAND_CMD_STATUS:
		...
		info->column = 0;
		info->addr = 0xFFFFFFFF;
		ret = dfc_send_command(mtd, flash_info->read_status, info->addr,
				       0, NDSR_RDDREQ);
#ifndef CONFIG_MTD_NAND_MONAHANS_DMA
	...
	case NAND_CMD_RESET:
		...
		if (status) {
			printk(KERN_WARNING
			       "Monahans NAND device: NAND_CMD_RESET error\n");
		}
		break;
	default:
		printk(KERN_WARNING
		       "Monahans NAND device: Non-support the command.\n");
		break;
	}
...
static void monahans_df_select_chip(struct mtd_info *mtd, int chip)
{
	...
	if (chip <= MAX_CHIP)
		...
		printk(KERN_ERR
		       "Monahans NAND device: not select the NAND chips!\n");
	}
}
...
static int monahans_df_calculate_ecc(struct mtd_info *mtd, const u_char * dat,
				     u_char * ecc_code)
{
	return 0;
}

static int monahans_df_correct_data(struct mtd_info *mtd, u_char * dat,
				    u_char * read_ecc, u_char * calc_ecc)
{
	 ...
	 * We have distinguish all the errors, but the nand_read_ecc
	 * only check this function return value
	 */
	if (info->retcode != ERR_NONE)
		return -1;
...
static int read_reloc_table(struct mtd_info *mtd)
{
	...
	if (valid) {
		...
		pr_debug((unsigned char *)&(info->table),
			  sizeof(struct reloc_table));
		info->table_init = 1;
	} else {
		/* There should be a valid relocation table slot at least. */
		printk(KERN_ERR
		       "NO VALID relocation table can be recognized\n");
...
{
	unsigned int bad_block = 0xFFFFFFFF;
	struct nand_chip *this = NULL;
	struct monahans_dfc_info *info = NULL;
	struct reloc_table *table = NULL;
...
static int monahans_df_probe(struct device *dev)
{
	...
	for (i = DFC_FLASH_NULL + 1; i < DFC_FLASH_END; i++) {
		...
		if (status)
			continue;
		printk(KERN_ERR "id:0x%x, chipid:0x%x\n", id,
		       dfc_context.flash_info->chip_id);
		if (id == dfc_context.flash_info->chip_id)
			break;
	}
	if (i == DFC_FLASH_END) {
		printk(KERN_ALERT
		       "Monahans NAND device: Nand Flash initialize failure!\n");
		ret = -ENXIO;
		goto out;
	}
	flash_config = i;
	...
	if (!monahans_mtd) {
		printk(KERN_ERR
		       "Monahans NAND device: Unable to allocate NAND MTD device structure.\n");
		ret = -ENOMEM;
		goto out;
	}
	...
	this->priv = info;
	data_buf_len =
	    dfc_context.flash_info->page_size +
	...
	/* alloc dma data buffer for data buffer + 2*descriptor + command buffer */
	buf_len = ALIGN(2 * sizeof(struct pxa_dma_desc), 32) +
	    ALIGN(data_buf_len, 32) + ALIGN(NAND_CMD_DMA_LEN, 32);

	printk(KERN_INFO
	       "Try to allocate dma buffer(len:%d) for data buffer + 2*descriptor + command buffer\n",
	       buf_len);
	info->data_desc =
	    (struct pxa_dma_desc *)dma_alloc_writecombine(dev, buf_len,
							  &info->data_desc_addr,
							  GFP_KERNEL);
	if (!info->data_desc) {
		printk(KERN_ERR
		       "Monahans NAND device: Unable to alloc dma buffer\n");
		ret = -ENOMEM;
		goto free_mtd;
	}
	...
	pr_debug
	   ("Get dma buffer for command dma descriptors, virt:0x%x, phys0x:%x\n",
	    (unsigned int)info->cmd_desc, info->cmd_desc_addr);
	pr_debug
	   ("Get dma buffer for data, virt:0x%x, phys0x:%x\n",
	    (unsigned int)info->data_buf, info->data_buf_addr);
	pr_debug
	   ("Get dma buffer for command, virt:0x%x, phys0x:%x\n",
	...
	if (info->data_dma < 0) {
		printk(KERN_ERR
		       "Monahans NAND device: Unable to alloc dma channel for data\n");
		ret = info->data_dma;
		goto free_buf;
	}
	...
	if (info->cmd_dma < 0) {
		printk(KERN_ERR
		       "Monahans NAND device: Unable to alloc dma channel for command\n");
		ret = info->cmd_dma;
		goto free_data_dma;
	}
...
#else
	printk(KERN_ERR "Try to allocate data buffer(len:%d)\n", data_buf_len);
	info->data_buf = kmalloc(data_buf_len, GFP_KERNEL);
	if (!info->data_buf) {
		printk(KERN_ERR
		       "Monahans NAND device: Unable to alloc data buffer\n");
		ret = -ENOMEM;
		goto free_mtd;
	}
#endif
...
#ifdef CONFIG_PM
static int monahans_df_suspend(struct device *dev, u32 state, u32 level)
{
	struct mtd_info *mtd = (struct mtd_info *)dev_get_drvdata(dev);
	struct monahans_dfc_info *info = (struct monahans_dfc_info *)
	    (((struct nand_chip *)(mtd->priv))->priv);
