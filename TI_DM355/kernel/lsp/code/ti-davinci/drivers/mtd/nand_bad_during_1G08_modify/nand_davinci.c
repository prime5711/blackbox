 ...
 */
//shcho change
// not boot after mtd info display
...
static spinlock_t nand_lock = SPIN_LOCK_UNLOCKED;
static unsigned long nand_flags,spin_lock_flags=0;
...
static void setup_nand_iomode(void)
{
	...
	__raw_writel(0x00000C40, pinmux2);   //__raw_writel(0x00000004, pinmux2);
}

static void setup_nand_cemode(void)
{
	void __iomem *pinmux2 =
		(void __iomem *) IO_ADDRESS(DAVINCI_SYSTEM_MODULE_BASE + 0x8);

	/* Configure the pin multiplexing to enable all of the EMIF signals */
	__raw_writel(0x00000C00, pinmux2);   //__raw_writel(0x00000004, pinmux2);
}
/*
...
static void nand_davinci_hwcontrol(struct mtd_info *mtd, int cmd)
{
    ...
    spin_lock_irqsave(&nand_lock,nand_flags);
    spin_lock_flags=1;
	  setup_nand_iomode();
  ...
  case NAND_CTL_CLRALE:
    ...
    setup_nand_cemode();
    if(spin_lock_flags){
        spin_unlock_irqrestore(&nand_lock,nand_flags);
        spin_lock_flags=0;
    }
		break;
	}
					 ...
					 u8 * page_data)
{
	...
	if (iserror == ECC_STATE_NO_ERR)
		...
		printk("ECC_STATE_TOO_MANY_ERRS\n");
		return -1;
		}	
