...
static unsigned long i2c_davinci_inputClock;	/* I2C input clock frequency in Hz */
 ...
 * also gets called if I2C encounters any errors. Clock calculation portion
...
static int i2c_davinci_reset(struct i2c_davinci_device *dev)
{
	...
	DEB1("CLK = %ld KHz", i2c_davinci_inputClock/(2*(psc + 1)*(clk + d)*1000));
