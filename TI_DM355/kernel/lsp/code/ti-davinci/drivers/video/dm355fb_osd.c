...
vpbe_bitmap_set_blend_factor(u8 * id, vpbe_bitmap_blend_params_t * blend_para)
{
	...
	if ((blend_para->enable_colorkeying > VPBE_ENABLE)
	    || (blend_para->bf > 7) || (blend_para->colorkey > 0xFFFFFF))
		return -VPBE_WRONG_WINDOW_ID;
	...
	if (is_win(id, OSD0)) {
		...
		dispc_reg_merge(OSD_OSDWIN0MD,
				...
				OSD_OSDWIN0MD_BLND0);
		if( (dm->osd0->info.var.bits_per_pixel == BITMAP_1BIT)
		|| (dm->osd0->info.var.bits_per_pixel == BITMAP_2BIT)
		|| (dm->osd0->info.var.bits_per_pixel == BITMAP_4BIT)
		|| (dm->osd0->info.var.bits_per_pixel == BITMAP_8BIT) ){
			dispc_reg_merge(OSD_TRANSPBMPIDX, 
					blend_para->colorkey << TRANSPBMPIDX_BMP0_SHIFT,
					TRANSPBMPIDX_BMP0);	
		}
		else if((dm->osd0->info.var.bits_per_pixel == YUV422_RGB565) 
					&& !is_rgb565(dm->osd0->info.var))
			dispc_reg_merge(OSD_TRANSPVALU, 
					blend_para->colorkey<<TRANSPVALU_Y_SHIFT, 
					TRANSPVALU_Y);
		else if((dm->osd0->info.var.bits_per_pixel == YUV422_RGB565) 
					&& is_rgb565(dm->osd0->info.var))
			dispc_reg_out(OSD_TRANSPVALL, blend_para->colorkey);
		else{ /* RGB888 */
			dispc_reg_out(OSD_TRANSPVALL, blend_para->colorkey & 0xffff);
			dispc_reg_merge(OSD_TRANSPVALU, 
					(blend_para->colorkey>>16) & 0xff, 
					TRANSPVALU_RGBU);
		}

		dm->osd0->conf_params.bitmap_params.blend_info =
		    *blend_para;
	} else {
		...
		dispc_reg_merge(OSD_OSDWIN1MD,
				...
				OSD_OSDWIN1MD_BLND1);
		if( (dm->osd1->info.var.bits_per_pixel == BITMAP_1BIT)
		|| (dm->osd1->info.var.bits_per_pixel == BITMAP_2BIT)
		|| (dm->osd1->info.var.bits_per_pixel == BITMAP_4BIT)
		|| (dm->osd1->info.var.bits_per_pixel == BITMAP_8BIT) ){
			dispc_reg_merge(OSD_TRANSPBMPIDX, 
					blend_para->colorkey << TRANSPBMPIDX_BMP1_SHIFT,
					TRANSPBMPIDX_BMP1);	
		}
		else if((dm->osd1->info.var.bits_per_pixel == YUV422_RGB565) 
					&& !is_rgb565(dm->osd1->info.var))
			dispc_reg_merge(OSD_TRANSPVALU, 
					blend_para->colorkey<<TRANSPVALU_Y_SHIFT, 
					TRANSPVALU_Y);
		else if((dm->osd1->info.var.bits_per_pixel == YUV422_RGB565) 
...
int vpbe_set_vid_params(u8 * id, vpbe_video_config_params_t * required)
{
	...
	dispc_reg_merge(OSD_MODE,
			...
			OSD_MODE_CS);
	dispc_reg_merge(OSD_MODE,
			required->exp_info.horizontal << OSD_MODE_VHRSZ_SHIFT,
			OSD_MODE_VHRSZ);
	dispc_reg_merge(OSD_MODE,
			required->exp_info.vertical << OSD_MODE_VVRSZ_SHIFT,
			OSD_MODE_VVRSZ);
	dispc_reg_merge(OSD_MODE,
			1<< OSD_MODE_EF_SHIFT,
			OSD_MODE_EF);
	...
	dm->vid0->conf_params.video_params.exp_info.horizontal 
		= required->exp_info.horizontal;
	dm->vid0->conf_params.video_params.exp_info.vertical 
		= required->exp_info.vertical;
	dm->vid1->conf_params.video_params.cb_cr_order 
		= required->cb_cr_order;
	dm->vid1->conf_params.video_params.exp_info.horizontal 
		= required->exp_info.horizontal;
	dm->vid1->conf_params.video_params.exp_info.vertical 
		= required->exp_info.vertical;
...
int vpbe_get_vid_params(u8 * id, vpbe_video_config_params_t * current_conf)
{
	...
	current_conf->cb_cr_order = dm->vid0->conf_params.video_params.cb_cr_order;
...
{
	/* While disabling the cursor no need to do error checking */
	if(!(required->enable)){
		dispc_reg_merge(OSD_RECTCUR, 0, OSD_RECTCUR_RCACT);
		dm->cursor = *required;
		return 0;
	}
	...
	if ((required->image.fg_color > 0xff)
	...
	dispc_reg_merge(OSD_RECTCUR, dm->cursor.enable ? OSD_RECTCUR_RCACT : 0,
		OSD_RECTCUR_RCACT);
