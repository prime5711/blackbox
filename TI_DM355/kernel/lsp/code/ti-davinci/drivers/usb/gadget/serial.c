...
static int gs_open(struct tty_struct *tty, struct file *file)
{
	...
	if (port_num < 0 || port_num >= GS_NUM_PORTS) {
		gs_debug("gs_open: (%d,%p,%p) invalid port number\n",
			port_num, tty, file);
		return -ENODEV;
	}
	...
	if (dev == NULL) {
		gs_debug("gs_open: (%d,%p,%p) NULL device pointer\n",
			port_num, tty, file);
		return -ENODEV;
	}
	...
	if (down_interruptible(sem)) {
		gs_debug("gs_open: (%d,%p,%p) interrupted waiting for semaphore\n",
			port_num, tty, file);
		return -ERESTARTSYS;
	}
	...
	if (dev->dev_config == GS_NO_CONFIG_ID) {
		gs_debug("gs_open: (%d,%p,%p) device is not connected\n",
			port_num, tty, file);
		ret = -ENODEV;
	...
	if (port == NULL) {
		gs_debug("gs_open: (%d,%p,%p) NULL port pointer\n",
			port_num, tty, file);
		ret = -ENODEV;
		goto exit_unlock_dev;
	}
	...
	if (port->port_dev == NULL) {
		gs_debug("gs_open: (%d,%p,%p) port disconnected (1)\n",
			port_num, tty, file);
		ret = -EIO;
	...
	if (port->port_write_buf == NULL) {
		...
		if (port->port_dev == NULL) {
			gs_debug("gs_open: (%d,%p,%p) port disconnected (2)\n",
				port_num, tty, file);
			port->port_in_use = 0;
			ret = -EIO;
			goto exit_unlock_port;
		...
		if ((port->port_write_buf=buf) == NULL) {
			gs_debug(KERN_ERR "gs_open: (%d,%p,%p) cannot allocate port write buffer\n",
				port_num, tty, file);
			port->port_in_use = 0;
			ret = -ENOMEM;
			goto exit_unlock_port;
	...
	/* might have been disconnected while asleep, check */
	if (port->port_dev == NULL) {
		gs_debug(KERN_ERR "gs_open: (%d,%p,%p) port disconnected (3)\n",
			port_num, tty, file);
		port->port_in_use = 0;
		ret = -EIO;
		goto exit_unlock_port;
...
static void gs_close(struct tty_struct *tty, struct file *file)
{
	...
	if (port == NULL) {
		gs_debug(KERN_ERR "gs_close: NULL port pointer\n");
		return;
	}
	...
	if (port->port_open_count == 0) {
		gs_debug("gs_close: (%d,%p,%p) port is already closed\n",
			port->port_num, tty, file);
		goto exit;
	}
...
static int gs_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	//printk("w%d", count);
	if (port == NULL) {
		gs_debug("gs_write: NULL port pointer\n");
		return -EIO;
	}
	...
	if (port->port_dev == NULL) {
		gs_debug("gs_write: (%d,%p) port is not connected\n",
			port->port_num, tty);
		ret = -EIO;
		goto exit;
	}
	...
	if (port->port_open_count == 0) {
		gs_debug(KERN_ERR "gs_write: (%d,%p) port is closed\n",
			port->port_num, tty);
		ret = -EBADF;
		goto exit;
	}
	...
	//printk("gs_write: (%d,%p) wrote %d bytes\n", port->port_num, tty,	count);
	//printk("W%d", count);
	return count;
...
static void gs_put_char(struct tty_struct *tty, unsigned char ch)
{
	...
	if (port == NULL) {
		gs_debug("gs_put_char: NULL port pointer\n");
		return;
	}
	...
	if (port->port_dev == NULL) {
		gs_debug("gs_put_char: (%d,%p) port is not connected\n",
			port->port_num, tty);
		goto exit;
	}

	if (port->port_open_count == 0) {
		gs_debug("gs_put_char: (%d,%p) port is closed\n",
			port->port_num, tty);
		goto exit;
	}
...
static void gs_flush_chars(struct tty_struct *tty)
{
	...
	gs_debug("FLUSH..................................\n");
	if (port == NULL) {
	  gs_debug("gs_flush_chars: NULL port pointer\n");
		return;
	}
	...
	if (port->port_dev == NULL) {
		gs_debug("gs_flush_chars: (%d,%p) port is not connected\n",
			port->port_num, tty);
		goto exit;
	}

	if (port->port_open_count == 0) {
		gs_debug("gs_flush_chars: (%d,%p) port is closed\n",
			port->port_num, tty);
		goto exit;
	}
...
static int gs_send(struct gs_dev *dev)
{
	...
	//if(list_empty(&dev->dev_req_list) == 1)
	//	printk("REQ LIST EMPTY...\n");
