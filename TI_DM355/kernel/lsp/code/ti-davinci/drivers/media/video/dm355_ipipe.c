...
int query_buffer(struct ipipe_device *device, struct ipipe_buffer *buffer)
{
	 ...
	 if ((buffer->buf_type != IPIPE_BUF_IN) && (buffer->buf_type != IPIPE_BUF_OUT)){
...
int request_buffer(struct ipipe_device *device, struct ipipe_reqbufs *reqbufs)
{
	...
	if ((reqbufs->buf_type != IPIPE_BUF_IN) && (reqbufs->buf_type != IPIPE_BUF_OUT)){
                ...
                for (count = device->out_numbuffers;
                     count < reqbufs->count; count++) {
                        ...
                        if (!buffer) {
                                ...
                                dev_err(ipipe_dev, "3.request_buffer:not enough \
					memory\n");
        ...
        if (device->params->ipipeif_param.source != 0) {
                ...
                if (arg->in_buff.index < 0) {
                        ...
                        if (arg->in_buff.offset % 32) {
                                dev_dbg(ipipe_dev, "address offset not alligned\n");
                                return -EINVAL;
                        }
                        /*lower sixteen bit */
        ...
        if (arg->out_buff.index < 0) {
                if (arg->out_buff.offset % 32)
                        ...
                        if (resizer_no_0 == 1 && resizer_no_1 == 1) {
                                printk("If both the resizer enable then user can \
...
static struct ipipe_def_cor_setup dfc_parm;
static struct ipipe_gamma_setup		gamma_parm;
static struct ipipe_prog_nf_setup	prog_nf_parm;
                ...
                unsigned int cmd, unsigned long arg)
{
	...
	case IPIPE_SET_WB:
		...
		struct ipipe_wb *wb_param = (struct ipipe_wb *)arg;
		wb_param->wb2_wg_r *=2;
		wb_param->wb2_wg_gr *=2;
		wb_param->wb2_wg_gb *=2;
		wb_param->wb2_wg_b *=2;
#endif
		set_wb_regs((struct ipipe_wb *)arg);

                up(&(device->sem));
                break;
	case IPIPE_SET_RSZ:
		set_ptz_regs((struct ipipe_rsz *)arg);
		break;

	case IPIPE_V_PAN:
		set_vpan_regs((unsigned int) arg);
		break;

	case IPIPE_H_PAN:
		set_hpan_regs((unsigned int) arg);
		break;
	...
	case IPIPE_RGB2RGB:
		...
		set_only_rgb_to_rgb_regs((struct ipipe_rgb2rgb *)arg);
	...
	case IPIPE_RGB2YUV:
		...
		set_only_rgb_to_yuv_regs((struct ipipe_rgb2yuv *)arg);
	...
	case IPIPE_BRT_CTR:
		...
		set_brt_ctr_regs((struct ipipe_rgb2yuv *)arg);
	...
	case IPIPE_FC_SUP:
		...
		set_fcs_regs((struct ipipe_false_color_suppresion *)arg);
	...
	case IPIPE_EDGE:
	{	
		//struct ipipe_edge_enhancer *edge_param = (struct ipipe_edge_enhancer *)arg;
		down_interruptible(&(device->sem));

		set_ee_regs((struct ipipe_edge_enhancer *)arg);
	...
	{	
		...
		set_ipipeif_gain((int)arg);

                up(&(device->sem));
	}
		break;
	case IPIPE_GAMMA_SETUP:
	{	
		//struct ipipe_edge_enhancer *edge_param = (struct ipipe_edge_enhancer *)arg;
		down_interruptible(&(device->sem));
		...
		else
		{
			...
			/* copy the parameters to the configuration */
                        if (copy_from_user(&gamma_parm, (struct ipipe_gamma_setup *)arg,
                             				sizeof(struct ipipe_gamma_setup))) 
			{
			...
			if(0)
			{
				...
				printk("gmm_cfg_bypr = %d \n",gamma_parm.gmm_cfg_bypr);
				printk("gmm_cfg_bypg = %d \n",gamma_parm.gmm_cfg_bypg);
				printk("gmm_cfg_bypb = %d \n",gamma_parm.gmm_cfg_bypb);
				printk("gmm_cfg_tbl = %d \n",gamma_parm.gmm_cfg_tbl);
				printk("gmm_cfg_siz = %d \n",gamma_parm.gmm_cfg_siz);
				for( cnt = 0;cnt < 1024;cnt ++ )
				{
					//printk("gmm_tbl_r = %d \n",gamma_parm.gmm_tbl_r[cnt]);
					//printk("gmm_tbl_b = %d \n",gamma_parm.gmm_tbl_b[cnt]);
					//printk("gmm_tbl_g = %d \n",gamma_parm.gmm_tbl_g[cnt]);
					printk("gmm_tbl_all = %d \n",gamma_parm.gmm_tbl_all[cnt]);
				}

			}

			set_gamma_regs(&gamma_parm);
		}
	...
	case IPIPE_NF_SETUP:
	{	
		struct ipipe_prog_nf parm_setup;
		...
		else
		{
			...
			/* copy the parameters to the configuration */
                        if (copy_from_user(&prog_nf_parm, (struct ipipe_prog_nf_setup *)arg,
                             				sizeof(struct ipipe_prog_nf_setup))) 
			{
                                /* if it fails return error */
                                up(&(device->sem));
                                return -EFAULT;
                        }
			parm_setup.noise_fil_en = prog_nf_parm.noise_fil_en ;
			parm_setup.d2f_cfg_spr 	= prog_nf_parm.d2f_cfg_spr;
			parm_setup.d2f_cfg_shf 	= prog_nf_parm.d2f_cfg_shf;
			parm_setup.type 	= prog_nf_parm.type; 
			parm_setup.d2f_thr 	= prog_nf_parm.d2f_thr;
			parm_setup.d2f_str	= prog_nf_parm.d2f_str;
			
			set_d2f_regs(&parm_setup);
		}
	...
	case IPIPE_DFC:
	{
		struct ipipe_params *pParms;

		down_interruptible(&(device->sem));
		/*default parameters */
                if ((void *)arg == NULL) 
		{
                        dfc_parm.dfc_en = DISABLE;
			dev_dbg(ipipe_dev, "NULL parameters.\n");
                } 
		else 
		{
                        /* copy the parameters to the configuration */
                        if (copy_from_user(&dfc_parm, (struct ipipe_def_cor_setup *)arg,
                             				sizeof(struct ipipe_def_cor_setup))) 
			{
                                /* if it fails return error */
                                up(&(device->sem));
                                return -EFAULT;
		...
		switch( dfc_parm.resolution )
		{
			case 0:
				pParms = &ipipe_mt9p031_VGA_30fps;
			break;
			case 1:
				pParms = &ipipe_mt9p031_VGA_BIN_30fps;
			break;
			case 2:
				pParms = &ipipe_mt9p031_720P;
			break;
			case 3:
				pParms = &ipipe_mt9p031_SXVGA;
			break;
			case 4:
				pParms = &ipipe_mt9p031_SXVGA;
			break;
			case 5:
				pParms = &ipipe_mt9p031_VGA_VGA;
			break;
			default:
				pParms = &param_def;
			break;
		}
		pParms->def_cor.dfc_en = dfc_parm.dfc_en;
		pParms->def_cor.dfc_sel = dfc_parm.dfc_sel;
		pParms->def_cor.dfc_siz = dfc_parm.dfc_siz;
		pParms->def_cor.dfc_adr = dfc_parm.dfc_adr;
		pParms->def_cor.dfc_table = dfc_parm.dfc_table;

		printk("\nIPIPE_DFC resolution = %d\n", dfc_parm.resolution );
		...
		dev_dbg(ipipe_dev, "Pass validation.\n");
                ...
                if (device->params)
                        ...
                        up(&(device->sem));
			dev_dbg(ipipe_dev, "invalid device parameter buffer.\n");
                        return -EINVAL;
                }
...
int __init ipipe_init(void)
{
	...
	class_simple_device_add(ipipe_class, dev, NULL, "dm355_ipipe");
        ...
        result = request_irq(IRQ_DM355_IPIPE_SDR, ipipe_isr, SA_SHIRQ/*SA_INTERRUPT*/,
