...
char *dm355_devices[] = {"MT9T001", "TVP5146", "MT9T031","MT9P031"};
...
static struct v4l2_rect SXGA_bounds = VPFE_WIN_SXGA;
static struct v4l2_rect SXVGA_bounds = VPFE_WIN_SXVGA;
static struct v4l2_rect P1080_bounds = VPFE_WIN_1080p;
static struct v4l2_rect P5M_bounds = VPFE_WIN_5MP;
...
static irqreturn_t v4l2Ipipe_isr(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	unsigned int 	offset1 	= offset+VPFE_EXT_HEADER_LEN;
	...
	if (frm_format == CCDC_FRMFMT_PROGRESSIVE) {
		if (!list_empty(&vpfe->dma_queue)
			...
			mirr_val = regr_ip(RSZ_SEQ);

			if(mirr_val == 0){
				write_out_addr(0, (unsigned int)vpfe->nextFrm->boff);
				write_out_addr(1, ((unsigned int)vpfe->nextFrm->boff)+offset1);
			}
			else if (mirr_val == 0x04){
				write_out_addr(0, ((unsigned int)vpfe->nextFrm->boff)+((vpfe->vwin.width*2)-1));
				write_out_addr(1, ((unsigned int)vpfe->nextFrm->boff)+offset1+(pResHeader->width*2)-1);
			}
			else if (mirr_val == 0x08){
				write_out_addr(0, ((unsigned int)vpfe->nextFrm->boff)+(vpfe->vwin.width*2*(vpfe->vwin.height-1)));
				write_out_addr(1, ((unsigned int)vpfe->nextFrm->boff)+offset1+(pResHeader->width*2*(pResHeader->height-1)));
			}
			else if (mirr_val == 0x0C){
				write_out_addr(0, ((unsigned int)vpfe->nextFrm->boff)+((vpfe->vwin.width*vpfe->vwin.height*2)-2));
				write_out_addr(1, ((unsigned int)vpfe->nextFrm->boff)+offset1+(pResHeader->width*2*pResHeader->height)-2);
			}
...
static irqreturn_t vdint1_isr(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (frm_format == CCDC_FRMFMT_PROGRESSIVE) {
		if (!list_empty(&vpfe->dma_queue)
			...
			list_del(&vpfe->nextFrm->queue);
			vpfe->nextFrm->state = STATE_ACTIVE;
			if(vpfe->ccdc_params_raw.alaw.b_alaw_enable) {

  			/* setup the memory address for the frame buffer */
  			ccdc_setfbaddr(((unsigned long)(vpfe->nextFrm->boff) + vpfe->ccdc_params_raw.win.width  ));
...
static void set_ext_stream(stream_ext_params 	*params, struct ipipe_params *config)
{
	...
	if( params->ext_res_num > 0 && params->ext_res_num < MAX_STREAM_EXT_NUM )
	{
		...
		int ratio_1	= (((config->rsz_rsc_param[0].rsz_o_hsz+1)*(config->rsz_rsc_param[0].rsz_h_dif)) )/res_width;
		int ratio_2	= (((config->rsz_rsc_param[0].rsz_o_vsz+1)*(config->rsz_rsc_param[0].rsz_v_dif)) )/res_height;
		int ratio_use;
			...
			unsigned int cmd, void *arg)
{
	...
	case VIDIOC_S_STD:
		{
			...
			if (device_type == TVP5146) {
				...
				if ((id == V4L2_STD_MT9T001_VGA_30FPS)
				...
				} else if (id == V4L2_STD_MT9T001_SXGA_30FPS){
					vpfe->std = id;
					vpfe->bounds = vpfe->vwin = SXGA_bounds;
					vpfe->pixelaspect = default_aspect;
					vpfe->ccdc_params_raw.win = SXGA_bounds;
				} else if (id == V4L2_STD_MT9T001_SXVGA_30FPS){
					vpfe->std = id;
					vpfe->bounds = vpfe->vwin = SXVGA_bounds;
					vpfe->pixelaspect = default_aspect;
					vpfe->ccdc_params_raw.win = SXVGA_bounds;
				} else if (id == V4L2_STD_MT9T001_VGA_VGA_30FPS){
					vpfe->std = id;
					vpfe->bounds = vpfe->vwin = SXVGA_bounds;
					vpfe->pixelaspect = default_aspect;
					vpfe->ccdc_params_raw.win = SXVGA_bounds;
					...
					vpfe->ccdc_params_raw.win =
					    P1080_bounds;
				} else if ((id == V4L2_STD_MT9T001_5MP_7FPS)) {
					vpfe->std = id;
					vpfe->bounds = vpfe->vwin = P5M_bounds;
					vpfe->pixelaspect = default_aspect;
					vpfe->ccdc_params_raw.win = P5M_bounds;
				}else {
					ret = -EINVAL;
				}
				if (ret != -EINVAL) {
	...
	case VIDIOC_ENUMSTD:
		{
			...
			if (device_type == TVP5146) {
				...
				if (index == 0) {
					...
					std->id = V4L2_STD_MT9T001_SXGA_30FPS;
					strcpy(std->name, "SXGA-1024line-30fps");
					std->framelines = FMT_SXGA_NUMLINES;
					std->frameperiod.numerator =
					    FPS_30_NUMERATOR;
					std->frameperiod.denominator =
					    FPS_30_DENOMIRATOR;
				} else if (index == 14) {
					std->id = V4L2_STD_MT9T001_SXVGA_30FPS;
					strcpy(std->name, "SXVGA-960line-30fps");
					std->framelines = FMT_SXVGA_NUMLINES;
					std->frameperiod.numerator =
					    FPS_30_NUMERATOR;
					std->frameperiod.denominator =
					    FPS_30_DENOMIRATOR;
				} else if (index == 15) {
					std->id = V4L2_STD_MT9T001_VGA_VGA_30FPS;
					strcpy(std->name, "SXVGA-600line-30fps");
					std->framelines = FMT_SXVGA_NUMLINES;
					std->frameperiod.numerator =
					    FPS_30_NUMERATOR;
					std->frameperiod.denominator =
				...
				if (device_type == TVP5146) {
						   ...
						   V4L2_STD_MT9T001_SXVGA_30FPS)
					{
						...
						capparm->timeperframe.
						   ...
						   std &
						   V4L2_STD_MT9T001_VGA_VGA_30FPS)
					{
						capparm->timeperframe.
						    numerator =
						    FPS_30_NUMERATOR;
	...
	case VIDIOC_S_CTRL:
		...
		if (device_type == TVP5146) {
			...
			else if (ctrl->id == V4L2_CID_MEDIAN)
			{
				...
				ccdc_wfc.done = 0;
				wait_for_completion_interruptible(&ccdc_wfc);
	...
	case VIDIOC_STREAMON:
		...
		if (device_type == TVP5146) {
			...
			ccdc_config_ycbcr(&vpfe->ccdc_params_ycbcr);
	  		...
	  		if(vpfe->pixelfmt == V4L2_PIX_FMT_SBGGR8 ) 
			{
	    		vpfe->ccdc_params_raw.alaw.b_alaw_enable = TRUE;
				vpfe->ccdc_params_raw.alaw.gama_wd = BITS_13_4;
			}
					...
					vpfe->pixelfmt == V4L2_PIX_FMT_YUYV)
		{
			if( device_type == MT9P031 )
			{
				...
				else if( vpfe->std  == V4L2_STD_MT9T001_SXVGA_30FPS )
				{
					config = &ipipe_mt9p031_SXVGA;
					config->ipipe_hst = 2;
					config->ipipe_vst = 13;
				}
				else if( vpfe->std  == V4L2_STD_MT9T001_VGA_VGA_30FPS )
				{
					config = &ipipe_mt9p031_VGA_VGA;
					config->ipipe_hst = 2;
					config->ipipe_vst = 13;
				}
				else if( vpfe->std  == V4L2_STD_MT9T001_SXGA_30FPS )
				{
					config = &ipipe_mt9p031_SXGA;
					config->ipipe_hst = 8;
					config->ipipe_vst = 1;
				}
			...
			{
				unsigned int mirr_val = regr_ip(RSZ_SEQ);
				unsigned int offset = vpfe->vwin.width*vpfe->vwin.height*2;
				unsigned int offset1 = offset+VPFE_EXT_HEADER_LEN;
			...
			if(vpfe->ccdc_params_raw.alaw.b_alaw_enable) {
  			...
  			/* setup the memory address for the frame buffer */
  			ccdc_setfbaddr(((unsigned long)(vpfe->curFrm->boff) + vpfe->ccdc_params_raw.win.width  ));
	...
	{
		if( device_type == MT9P031 || device_type == MT9T001 )
		{
...
static int vpfe_init(void)
{
	...
	if (device_type == TVP5146) {
		...
		ret = vpfe_device.config_dev_fxn(MT9T001_INIT,
					       &vpfe_device.std,
					       &vpfe_device.device_params);
	}
	...
	if (frame_format == CCDC_FRMFMT_PROGRESSIVE) {
		if( (device_type != TVP5146) &&
					vpfe_device.pixelfmt == V4L2_PIX_FMT_YUYV)
		{
			irq_setup_by_fmt(IRQ_USE_IPIPE);
		}else{
			irq_setup_by_fmt(IRQ_USE_VD1);
