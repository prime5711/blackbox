...
char *dm355_devices[] = {"MT9T001", "TVP5146", "MT9T031","MT9P031","TVP5150","MT9M131","TVP7002"};
	...
	while ((this_opt = strsep(&options, ":")) != NULL) {
		...
		if (!strncmp(this_opt, "device=", 6)) 
		  ...
		  if (!strncmp(this_opt + 7, "MT9T031", 7))
			{
				printk("MT9T031 selected by xact\n");
				device_type = MT9T031;
			}
			else if (!strncmp(this_opt + 7, "MT9T001", 7))
			{
				printk("MT9T001 selected by xact\n");
				device_type = MT9T001;
			}
			else if (!strncmp(this_opt + 7, "TVP5146", 7))
			{
				printk("TVP5146 selected by xact\n");
				device_type = TVP5146;
			}
			else if (!strncmp(this_opt + 7, "MT9P031", 7))
			{
				printk("MT9P031 selected by xact\n");
				device_type = MT9P031;
			}
			else if (!strncmp(this_opt + 7, "TVP5150", 7))
			{
				printk("TVP5150 selected by xact\n");
				device_type = TVP5150;
			}
			else if (!strncmp(this_opt + 7, "MT9M131", 7))
			{
				printk("MT9M131 selected by xact\n");
				device_type = MT9M131;
			}
...
static vpfe_obj vpfe_device_ycbcr_mt9m131 = {	/* the default format is NTSC */
	...
	.pixelaspect = VPFE_PIXELASPECT_DEFAULT, //VPFE_PIXELASPECT_NTSC ==> VPFE_PIXELASPECT_DEFAULT
...
//				printk("VIDIOC_S_FMT in MT9P031\n");
			...
			if((device_type == TVP5150)||(device_type == MT9M131)||(device_type == TVP7002))
			{
	...
	case VIDIOC_S_STD:
		{
			...
			else if (device_type == TVP5150) 
			{
				if (id & V4L2_STD_625_50) 
				{
					...
					#if DM355_VPFE_DEBUG
					//printk("id & VPFE_STD_AUTO_SQP by xact\n");
					#endif
					vpfe->std = id;
					vpfe->bounds = vpfe->vwin =
					    palsp_bounds;
				...
				{
					//printk("else return error by xact\n");
					ret = -EINVAL;
				}
				if (id == V4L2_STD_PAL_60) {
					#if DM355_VPFE_DEBUG
					//printk("id == V4L2_STD_PAL_60 by xact\n");
					#endif
					mode = TVP5150_MODE_PAL_60;
				} else if (id == V4L2_STD_PAL_M) {
					#if DM355_VPFE_DEBUG
					//printk("id == V4L2_STD_PAL_M by xact\n");
					#endif
					mode = TVP5150_MODE_PAL_M;
				} else if (id == V4L2_STD_PAL_Nc
					   || id == V4L2_STD_PAL_N) {
					#if DM355_VPFE_DEBUG
					//printk("id == V4L2_STD_PAL_N by xact\n");
					#endif
					mode = TVP5150_MODE_PAL_CN;
				} else if (id & V4L2_STD_PAL) {
					#if DM355_VPFE_DEBUG
					//printk("id & V4L2_STD_PAL by xact\n");
					#endif
					mode = TVP5150_MODE_PAL;
				} else if (id & V4L2_STD_NTSC) {
					#if DM355_VPFE_DEBUG
					//printk("id & V4L2_STD_NTSC by xact\n");
					#endif
					mode = TVP5150_MODE_NTSC;
				} else if (id & V4L2_STD_SECAM) {
					#if DM355_VPFE_DEBUG
					//printk("id & V4L2_STD_SECAM by xact\n");
					#endif
			...
			if (device_type == MT9M131) 
			{
				 ...
				 */
				printk("in VIDIOC_S_STD ioctl device_type == MT9M131 by xact\n");
				...
				else if(id == MT9M131_MODE_720p_24FPS)
				{
					printk("id ==> 720p_30FPS\n");
					vpfe->std = id;
					vpfe->bounds = vpfe->vwin = P720_bounds; //VPFE_WIN_720p
					vpfe->pixelaspect = default_aspect;
					vpfe->ccdc_params_ycbcr.win = P720_bounds;	
				}	
				else if(id == MT9M131_MODE_SXVGA_960)
				{
					printk("id ==> 1280x960 MT9M131\n");
					vpfe->std = id;
					vpfe->bounds = vpfe->vwin = SXGA_960_bounds; //VPFE_WIN_SXVGA 1280x1024
					vpfe->pixelaspect = default_aspect;
					vpfe->ccdc_params_ycbcr.win = SXGA_960_bounds;	
				}
				else if(id == MT9M131_MODE_SXVGA_15FPS)
				{
					printk("id ==> SXVGA_15FPS\n");
					vpfe->std = id;
			...
			else if (device_type == MT9T001 || device_type == MT9T031 ||device_type == MT9P031)
			{
				 ...
				 * passed by application
				 */
				//printk("in VIDIOC_S_STD ioctl device_type == MT9P031 by xact\n");
	...
	case VIDIOC_G_PARM:
		{
			...
			printk("777777777777 VIDIOC_G_PARM\n");
	...
	case VIDIOC_QUERYSTD:
		{
			...
			else 
			{
				 ...
				 * is not supported
				 */
				printk("VIDIOC_QUERYSTD not used by xact\n");
				ret = -EINVAL;
			}
	...
	case VIDIOC_REQBUFS:
		...
		if (vpfe->io_usrs != 0) 
		{
			printk("RRRRRRRRRRRRRRRRRRRRRR EBUSY\n");
			ret = -EBUSY;
			break;
		}
		down_interruptible(&vpfe->lock);
	...
	case VIDIOC_STREAMON:
		//printk("Starting VIDIOC_STREAMON ioctl by xact\n");
		...
		if (device_type == MT9M131) 	
		{
			...
			printk("%%%%%% VIDIOC_STREAMON MT9M131 ccdc_config_ycbcr by xact\n");
		...
		else 
		{
			...
			/* configure the ccdc and resizer as needed   */
			/* start capture by enabling CCDC and resizer */
			//printk("ccdc_config_raw by xact\n");
	...
	{
		if( device_type == MT9P031 || device_type == MT9T001 || device_type == TVP5150 || device_type == MT9M131 || device_type == TVP7002)
		{
...
static int __init vpfe_probe(struct device *device)
{
	...
	if ((vfd = video_device_alloc()) == NULL) {
		printk("vpfe_probe error by xact\n");
		return -ENOMEM;
	}
	//printk("vpfe_probe OK by xact\n");
	*vfd = vpfe_video_template;
	...
	if (video_register_device(vpfe->video_dev, VFL_TYPE_GRABBER, -1) < 0) {
		printk("video_register_device error by xact\n");
...
static int vpfe_init(void)
{
	...
	{
		printk("vpfe_driver driver registration failed by xact\n");
	...
	{
		printk("_vpfe_device platform_device_register failed by xact\n");
	...
	if (ret < 0) 
	{
		printk("RRRRRRRRRRRRRR request_irq Error\n");
		platform_device_unregister(&_vpfe_device);
		driver_unregister(&vpfe_driver);
		/* Free memory for all image buffers */
		for (i = 0; i < VPFE_DEFNUM_FBUFS; i++) {
