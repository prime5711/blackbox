...
#if defined(CONFIG_BCM4704) || defined(CONFIG_BCM4710) || defined(CONFIG_BCM5365)
...
/* Wish __BUILD_SET_CP0 did this... */
#define read_c0_diagnostic()	 __read_32bit_c0_register($22, 0)
#define write_c0_diagnostic(val) __write_32bit_c0_register($22, 0, val)

__BUILD_SET_C0(diagnostic)
...
static void __init _change_cachability(u32 cm)
{
	...
	if (BCM330X(current_cpu_data.processor_id)) {
		...
		set_c0_diagnostic((1 << 31) | (1 << 30));
	}
}
static void (*change_cachability)(u32);

static inline void fill_icache_line(unsigned long addr)
{
	cache_op(Fill, addr);
}
...
{
	int i;
	for (i = 0; i < nbytes; i += current_cpu_data.icache.linesz)
...
#endif /*defined(CONFIG_BCM4704) || defined(CONFIG_BCM4710) || defined(CONFIG_BCM5365)*/

static inline void coherency_setup(void)
{
#if defined(CONFIG_BCM4704) || defined(CONFIG_BCM4710) || defined(CONFIG_BCM5365)
	change_cachability =
		(void (*)(u32))KSEG1ADDR((unsigned long)(_change_cachability));
...
void __init ld_mmu_r4xx0(void)
{
	...
	if (BCM330X(c->processor_id) &&
		(read_c0_diagnostic() & (1 << 29))) {
