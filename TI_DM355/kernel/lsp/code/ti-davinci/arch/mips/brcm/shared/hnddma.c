...
void *dma_attach(void *drv, void *dev, char *name, dmaregs_t * regs,
		 uint ntxd, uint nrxd, uint rxbufsize, uint nrxpost,
		 uint rxoffset, uint ddoffset, uint dataoffset,
		 uint * msg_level)
{
	dma_info_t *di;
	void *va;
...
void dma_txreset(dma_info_t * di)
{
	uint32 status;
	...
	SPINWAIT((status =
		  (R_REG(&di->regs->xmtstatus) & XS_XS_MASK)) !=
...
void dma_rxreset(dma_info_t * di)
{
	uint32 status;
	...
	SPINWAIT((status =
		  (R_REG(&di->regs->rcvstatus) & RS_RS_MASK)) !=
...
bool dma_txenabled(dma_info_t * di)
{
	uint32 xc;
...
void dma_txsuspend(dma_info_t * di)
{
	DMA_TRACE(("%s: dma_txsuspend\n", di->name));
	OR_REG(&di->regs->xmtcontrol, XC_SE);
}

void dma_txresume(dma_info_t * di)
{
	DMA_TRACE(("%s: dma_txresume\n", di->name));
	AND_REG(&di->regs->xmtcontrol, ~XC_SE);
}

bool dma_txsuspended(dma_info_t * di)
{
	uint32 xc;
	uint32 xs;
...
bool dma_txstopped(dma_info_t * di)
{
	return ((R_REG(&di->regs->xmtstatus) & XS_XS_MASK) ==
		XS_XS_STOPPED);
}

bool dma_rxstopped(dma_info_t * di)
{
	return ((R_REG(&di->regs->rcvstatus) & RS_RS_MASK) ==
		RS_RS_STOPPED);
}

void dma_fifoloopbackenable(dma_info_t * di)
{
	DMA_TRACE(("%s: dma_fifoloopbackenable\n", di->name));
	OR_REG(&di->regs->xmtcontrol, XC_LE);
}

void dma_rxinit(dma_info_t * di)
{
...
void dma_rxenable(dma_info_t * di)
{
	DMA_TRACE(("%s: dma_rxenable\n", di->name));
	W_REG(&di->regs->rcvcontrol,
	      ((di->rxoffset << RC_RO_SHIFT) | RC_RE));
}

bool dma_rxenabled(dma_info_t * di)
{
	uint32 rc;
 ...
 */
int dma_txfast(dma_info_t * di, void *p0, uint32 coreflags)
{
	void *p, *next;
	uchar *data;
	uint len;
	...
	for (p = p0; p; p = next) {
		...
		W_SM(&di->txd[txout].addr,
		     BUS_SWAP32(pa + di->dataoffset));
...
#define	PAGEBASE(x)	((uint)(x) & PAGE_MASK)
 ...
 */
int dma_tx(dma_info_t * di, void *p0, uint32 coreflags)
{
	void *p, *next;
	uchar *data;
	uint plen, len;
	...
	for (p = p0; p; p = next) {
		...
		for (page = (uchar *) PAGEBASE(data);
			...
			start =
			    (page ==
			     (uchar *) PAGEBASE(data)) ? data : page;
			end =
			    (page ==
			     (uchar *) PAGEBASE(data + plen)) ? (data +
								 plen)
			    : (page + PAGE_SIZE);
			len = end - start;
			...
			/* get physical address of buffer start */
			pa = (uint32) DMA_MAP(di->dev, start, len, DMA_TX,
					      p);
...
void dma_rxfill(dma_info_t * di)
{
	...
	for (i = 0; i < n; i++) {
		...
		*(uint32 *) (OSL_UNCACHED(PKTDATA(di->drv, p))) = 0;
		...
		pa = (uint32) DMA_MAP(di->dev, PKTDATA(di->drv, p),
				      rxbufsize, DMA_RX, p);
...
void dma_txreclaim(dma_info_t * di, bool forceall)
{
	void *p;
 ...
 */
void *dma_getnexttxp(dma_info_t * di, bool forceall)
{
	uint start, end, i;
	void *txp;
	...
	for (i = start; i != end && !txp; i = NEXTTXD(i)) {
		DMA_UNMAP(di->dev,
			  (BUS_SWAP32(R_SM(&di->txd[i].addr)) -
			   di->dataoffset),
			  (BUS_SWAP32(R_SM(&di->txd[i].ctrl)) &
			   CTRL_BC_MASK), DMA_TX, di->txp[i]);
...
void dma_rxreclaim(dma_info_t * di)
{
	void *p;
...
void *dma_getnextrxp(dma_info_t * di, bool forceall)
{
	uint i;
	void *rxp;
	...
	if (!forceall
	    && (i == B2I(R_REG(&di->regs->rcvstatus) & RS_CD_MASK)))
		return (NULL);
...
uint dma_getvar(dma_info_t * di, char *name)
{
...
void dma_txblock(dma_info_t * di)
{
	di->txavail = 0;
}

void dma_txunblock(dma_info_t * di)
{
	di->txavail = di->ntxd - NTXDACTIVE(di->txin, di->txout) - 1;
}

uint dma_txactive(dma_info_t * di)
{
