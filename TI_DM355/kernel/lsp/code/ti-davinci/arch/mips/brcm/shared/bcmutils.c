...
ulong bcm_strtoul(char *cp, char **endp, uint base)
{
	...
	while (bcm_isxdigit(*cp) &&
	       (value =
		bcm_isdigit(*cp) ? *cp - '0' : bcm_toupper(*cp) - 'A' +
		10) < base) {
...
void prhex(char *msg, uchar * buf, uint nbytes)
{
	char line[256];
	char *p;
	uint i;
...
uint pktcopy(void *drv, void *p, uint offset, int len, uchar * buf)
{
	uint n, ret = 0;
...
uchar *bcm_ether_ntoa(char *ea, char *buf)
{
	sprintf(buf, "%02x:%02x:%02x:%02x:%02x:%02x",
		(uchar) ea[0] & 0xff, (uchar) ea[1] & 0xff,
		(uchar) ea[2] & 0xff, (uchar) ea[3] & 0xff,
		(uchar) ea[4] & 0xff, (uchar) ea[5] & 0xff);
	return (buf);
}
...
void pktqinit(struct pktq *q, int maxlen)
{
	q->head = q->tail = NULL;
	q->maxlen = maxlen;
	q->len = 0;
...
uint16 crc16(uint8 * pdata,	/* pointer to array of data to process */
	     uint nbytes,	/* number of input data bytes to process */
