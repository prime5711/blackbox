...
/*
 * Returns TRUE if an external UART exists at the given base register.
 */
static bool serial_exists(uint8 * regs)
{
 ...
 */
void
sb_serial_init(void *sbh,
	       void (*add) (void *regs, uint irq, uint baud_base,
			    uint reg_shift))
{
	void *regs;
	ulong base;
...
static void sb_clearirq(void *sbh, uint irq)
{
	void *regs;
	sbconfig_t *sb;
 ...
 */
static void sb_setirq(void *sbh, uint irq, uint coreid, uint coreunit)
{
	void *regs;
	sbconfig_t *sb;
	uint32 flag;
...
static void icache_probe(int *size, int *lsize)
{
	uint32 config1;
	uint sets, ways;
 ...
 */
bool
sb_mips_setclock(void *sbh, uint32 mipsclock, uint32 sbclock,
		 uint32 pciclock)
{
	...
	volatile uint32 *clockcontrol_n, *clockcontrol_sb,
	    *clockcontrol_pci;
	uint32 orig_n, orig_sb, orig_pci, orig_m2, orig_mips,
	    orig_ratio_parm, new_ratio;
	...
	n4m_table_t type2_table[] = {
		{180000000, 80000000, 0x0403, 0x01010000, 0x01020300,
		 0x01020600, 0x05000100, 0x94, 0x012a0115},
	...
	if (pll_type == PLL_TYPE1) {
		...
		if (pciclock == 0) {
			pciclock =
			    sb_clock_rate(pll_type, R_REG(clockcontrol_n),
					  R_REG(clockcontrol_pci));
			pciclock =
			    (pciclock <= 25000000) ? 25000000 : 33000000;
		}
		...
		n4m_table_t *table =
		    (pll_type == PLL_TYPE2) ? type2_table : type4_table;
		uint tabsz =
		    (pll_type ==
		     PLL_TYPE2) ? ARRAYSIZE(type2_table) :
...
uint32 sb_memc_get_ncdl(void *sbh)
{
	sbmemcregs_t *memc;
	uint32 ret = 0;
