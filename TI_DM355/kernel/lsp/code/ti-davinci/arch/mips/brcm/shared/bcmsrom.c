...
#include <asm/brcm/proto/ethernet.h>	/* for sprom content groking */
...
static int sprom_read_pci(uint16 * sprom, uint byteoff, uint16 * buf,
			  uint nbytes, bool check_crc);

/*
 * Initialize the vars from the right source for this platform.
 * Return 0 on success, nonzero on error.
 */
int srom_var_init(uint bus, void *curmap, void *osh, char **vars,
		  int *count)
{
	if (vars == NULL)
		return (0);
...
int
srom_read(uint bus, void *curmap, void *osh, uint byteoff, uint nbytes,
	  uint16 * buf)
{
	void *srom;
	uint i, off, nw;
...
int
srom_write(uint bus, void *curmap, void *osh, uint byteoff, uint nbytes,
	   uint16 * buf)
{
	uint16 *srom;
	...
	crc_range =
	    ((bus == PCMCIA_BUS) ? SPROM_SIZE : SPROM_CRC_RANGE) * 2;

	/* if changes made inside crc cover range */
	if (byteoff < crc_range) {
		nw = (((byteoff + nbytes) >
		       crc_range) ? byteoff + nbytes : crc_range) / 2;
		...
		crc =
		    ~crc8((uint8 *) image, crc_range - 1, CRC8_INIT_VALUE);
		ltoh16_buf(image, crc_range);
		image[(crc_range / 2) - 1] =
		    (crc << 8) | (image[(crc_range / 2) - 1] & 0xff);
		p = image;
		off = 0;
	} else {
		p = buf;
	...
	if (bus == PCI_BUS) {
		...
		val32 =
		    OSL_PCI_READ_CONFIG(osh, PCI_SPROM_CONTROL,
					sizeof(uint32));
		val32 |= SPROM_WRITEEN;
...
int srom_parsecis(uint8 * cis, char **vars, int *count)
{
	char eabuf[32];
	char *vp, *base;
	...
	do {
		...
		case CISTPL_BRCM_HNBU:
			...
			case HNBU_LED:
				if (cis[i + 1] != 0xff) {
					vp +=
					    sprintf(vp, "wl0gpio0=%d",
						    cis[i + 1]);
					vp++;
				}
				if (cis[i + 2] != 0xff) {
					vp +=
					    sprintf(vp, "wl0gpio1=%d",
						    cis[i + 2]);
					vp++;
				}
				if (cis[i + 3] != 0xff) {
					vp +=
					    sprintf(vp, "wl0gpio2=%d",
						    cis[i + 3]);
					vp++;
				}
				if (cis[i + 4] != 0xff) {
					vp +=
					    sprintf(vp, "wl0gpio3=%d",
						    cis[i + 4]);
					vp++;
				}
				break;
 ...
 */
static int
sprom_read_pci(uint16 * sprom, uint byteoff, uint16 * buf, uint nbytes,
	       bool check_crc)
{
	int off, nw;
	uint8 chk8;
	...
	if (check_crc) {
		...
		htol16_buf(buf, nw * 2);
		if ((chk8 =
		     crc8((uchar *) buf, nbytes,
			  CRC8_INIT_VALUE)) != CRC8_GOOD_VALUE)
			return (1);
...
static int initvars_srom_pci(void *curmap, char **vars, int *count)
{
	...
	if (sprom_read_pci
	    ((void *) ((uint) curmap + PCI_BAR0_SPROM_OFFSET), 0, b,
	     sizeof(b), TRUE))
		return (-1);
