...
#include <asm/brcm/sbpcmcia.h>
#include <asm/brcm/sbextif.h>
#include <asm/brcm/sbutils.h>
#include <asm/brcm/bcmsrom.h>
...
/* local prototypes */
static void *sb_doattach(sb_info_t * si, uint devid, void *osh, void *regs,
			 uint bustype, void *sdh, char **vars, int *varsz);
static void sb_scan(sb_info_t * si);
static uint sb_corereg(void *sbh, uint coreidx, uint regoff, uint mask,
		       uint val);
...
static uint32 sb_read_sbreg(void *sbh, volatile uint32 * sbr)
{
	sb_info_t *si;
	uint8 tmp;
	...
	/*
	 * CompactFlash only has 11-bit address, while we need 12-bit address.
	 * MEM_SEG will be OR'd with other 11 bits of address in hardware, so we
	 * program MEM_SEG with 12th bit when necessary (access to SB registers).
...
static void sb_write_sbreg(void *sbh, volatile uint32 * sbr, uint32 v)
{
	sb_info_t *si;
	uint8 tmp;
...
#ifdef IL_BIGENDIAN
		dummy = R_REG(sbr);
		W_REG((volatile uint16 *) ((uint32) sbr + 2),
		      (uint16) ((v >> 16) & 0xffff));
		dummy = R_REG(sbr);
 ...
 */
void *sb_attach(uint devid, void *osh, void *regs, uint bustype, void *sdh,
		char **vars, int *varsz)
{
	sb_info_t *si;
	...
	return (sb_doattach
		(si, devid, osh, regs, bustype, sdh, vars, varsz));
}
			 ...
			 uint bustype, void *sdh, char **vars, int *varsz)
{
	...
	if (cc) {
		...
		si->chiprev =
		    (R_REG(&cc->chipid) & CID_REV_MASK) >> CID_REV_SHIFT;
		si->chippkg =
		    (R_REG(&cc->chipid) & CID_PKG_MASK) >> CID_PKG_SHIFT;
	} else {
		...
		if (si->bus == PCMCIA_BUS) {
			...
			w = OSL_PCI_READ_CONFIG(osh, PCI_CFG_REV,
						sizeof(uint32));
			si->chiprev = w & 0xff;
		} else
			si->chiprev = 0;
	...
	if (cc && (sb_corerev((void *) si) >= 4))
		si->numcores =
		    (R_REG(&cc->chipid) & CID_CC_MASK) >> CID_CC_SHIFT;
	else
...
uint sb_coreid(void *sbh)
{
	...
	return ((R_SBREG(sbh, &(sb)->sbidhigh) & SBIDH_CC_MASK) >>
		SBIDH_CC_SHIFT);
}

uint sb_coreidx(void *sbh)
{
	sb_info_t *si;
...
static uint _sb_coreidx(void *sbh)
{
	...
	case PCI_BUS:
		sbaddr =
		    OSL_PCI_READ_CONFIG(si->osh, PCI_BAR0_WIN,
					sizeof(uint32));
		break;
	...
	case PCMCIA_BUS:{
			...
			OSL_PCMCIA_READ_ATTR(si->osh, PCMCIA_ADDR0, &tmp,
					     1);
			sbaddr = (uint) tmp << 12;
			OSL_PCMCIA_READ_ATTR(si->osh, PCMCIA_ADDR1, &tmp,
					     1);
			sbaddr |= (uint) tmp << 16;
			OSL_PCMCIA_READ_ATTR(si->osh, PCMCIA_ADDR2, &tmp,
					     1);
			sbaddr |= (uint) tmp << 24;
			break;
		}
...
uint sb_corevendor(void *sbh)
{
	...
	return ((R_SBREG(sbh, &(sb)->sbidhigh) & SBIDH_VC_MASK) >>
		SBIDH_VC_SHIFT);
}

uint sb_corerev(void *sbh)
{
	sb_info_t *si;
	sbconfig_t *sb;
...
uint32 sb_coreflags(void *sbh, uint32 mask, uint32 val)
{
	sb_info_t *si;
	sbconfig_t *sb;
	uint32 w;
...
uint32 sb_coreflagshi(void *sbh, uint32 mask, uint32 val)
{
	sb_info_t *si;
	sbconfig_t *sb;
	uint32 w;
...
bool sb_iscoreup(void *sbh)
{
	...
	return ((R_SBREG(sbh, &(sb)->sbtmstatelow) &
...
void *sb_setcoreidx(void *sbh, uint coreidx)
{
	sb_info_t *si;
	uint32 sbaddr;
	uint8 tmp;
...
void *sb_setcore(void *sbh, uint coreid, uint coreunit)
{
	sb_info_t *si;
	uint idx;
...
uint sb_boardstyle(void *sbh)
{
	...
	if ((srom_read
	     (si->bus, si->curmap, si->osh, (SPROM_SIZE - 1) * 2, 2,
	      &w) == 0)
	    && (w == 0x0313))
		return (BOARDSTYLE_CARDBUS);
...
uint sb_corelist(void *sbh, uint coreid[])
{
	sb_info_t *si;
...
bool sb_taclear(void *sbh)
{
	...
	if (si->bus == PCI_BUS) {
		...
		stcmd =
		    OSL_PCI_READ_CONFIG(si->osh, PCI_CFG_CMD,
					sizeof(stcmd));
...
void sb_core_reset(void *sbh, uint32 bits)
{
	sb_info_t *si;
	sbconfig_t *sb;
	volatile uint32 dummy;
...
void sb_core_disable(void *sbh, uint32 bits)
{
	sb_info_t *si;
	volatile uint32 dummy;
	sbconfig_t *sb;
...
void sb_watchdog(void *sbh, uint ticks)
{
	...
	switch (si->gpioid) {
	case SB_CC:
		sb_corereg(sbh, si->gpioidx,
			   OFFSETOF(chipcregs_t, watchdog), ~0, ticks);
		break;
	case SB_EXTIF:
		sb_corereg(sbh, si->gpioidx,
			   OFFSETOF(extifregs_t, watchdog), ~0, ticks);
		break;
	}
}

/* initialize the pcmcia core */
void sb_pcmcia_init(void *sbh)
{
	sb_info_t *si;
	uint8 cor;
	...
	if (si->chip == BCM4301_DEVICE_ID) {
		...
		OSL_PCMCIA_READ_ATTR(si->osh, PCMCIA_FCR2 + PCMCIA_COR,
				     &cor, 1);
		cor |= COR_IRQEN | COR_FUNEN;
		OSL_PCMCIA_WRITE_ATTR(si->osh, PCMCIA_FCR2 + PCMCIA_COR,
				      &cor, 1);
	} else {
		OSL_PCMCIA_READ_ATTR(si->osh, PCMCIA_FCR0 + PCMCIA_COR,
				     &cor, 1);
		cor |= COR_IRQEN | COR_FUNEN;
		OSL_PCMCIA_WRITE_ATTR(si->osh, PCMCIA_FCR0 + PCMCIA_COR,
				      &cor, 1);
	}
 ...
 */
void sb_pci_setup(void *sbh, uint32 * dmaoffset, uint coremask)
{
	sb_info_t *si;
	sbconfig_t *sb;
	sbpciregs_t *pciregs;
	...
	if (si->pcirev < 6) {
		...
		w = OSL_PCI_READ_CONFIG(si->osh, PCI_INT_MASK,
					sizeof(uint32));
		w |= (coremask << PCI_SBIM_SHIFT);
		OSL_PCI_WRITE_CONFIG(si->osh, PCI_INT_MASK, sizeof(uint32),
				     w);
	}
...
uint32 sb_size(uint32 admatch)
{
	...
	if (type == 0) {
		size =
		    1 <<
		    (((admatch & SBAM_ADINT0_MASK) >> SBAM_ADINT0_SHIFT) +
		     1);
	} else if (type == 1) {
		ASSERT(!(admatch & SBAM_ADNEG));	/* neg not supported */
		size =
		    1 <<
		    (((admatch & SBAM_ADINT1_MASK) >> SBAM_ADINT1_SHIFT) +
		     1);
	} else if (type == 2) {
		ASSERT(!(admatch & SBAM_ADNEG));	/* neg not supported */
		size =
		    1 <<
		    (((admatch & SBAM_ADINT2_MASK) >> SBAM_ADINT2_SHIFT) +
		     1);
	}
...
static INLINE uint32 factor6(uint32 x)
{
	switch (x) {
	case CC_F6_2:
		return 2;
...
uint32 sb_clock_rate(uint32 pll_type, uint32 n, uint32 m)
{
...
uint32 sb_gpiocontrol(void *sbh, uint32 mask, uint32 val)
{
	sb_info_t *si;
	uint regoff;
...
uint32 sb_gpioouten(void *sbh, uint32 mask, uint32 val)
{
	sb_info_t *si;
	uint regoff;
...
uint32 sb_gpioout(void *sbh, uint32 mask, uint32 val)
{
	sb_info_t *si;
	uint regoff;
...
uint32 sb_gpiointpolarity(void *sbh, uint32 mask, uint32 val)
{
	sb_info_t *si;
	uint regoff;
...
uint32 sb_gpiointmask(void *sbh, uint32 mask, uint32 val)
{
	sb_info_t *si;
	uint regoff;
 ...
 */
static uint slowfreq(void *sbh, bool max)
{
	sb_info_t *si;
	chipcregs_t *cc;
	uint32 v;
	...
	if (si->ccrev < 6) {
		v = OSL_PCI_READ_CONFIG(si->osh, PCI_GPIO_OUT,
					sizeof(uint32));

		if (v & PCI_CFG_GPIO_SCS)
			return (max ? (PCIMAXFREQ / 64)
				: (PCIMINFREQ / 64));
		else
			return (max ? (XTALMAXFREQ / 32)
				: (XTALMINFREQ / 32));
	} else {
		...
		if (v == SCC_SS_LPO)
			...
			return (max ? (XTALMAXFREQ / div)
				: (XTALMINFREQ / div));
		else if (v == SCC_SS_PCI)
			return (max ? (PCIMAXFREQ / div)
				: (PCIMINFREQ / div));
		else
			ASSERT(0);
	}
...
uint16 sb_pwrctl_fast_pwrup_delay(void *sbh)
{
	sb_info_t *si;
	uint origidx;
	chipcregs_t *cc;
	...
	fpdelay =
	    (((R_REG(&cc->pll_on_delay) + 2) * 1000000) +
...
int sb_pwrctl_xtal(void *sbh, uint what, bool on)
{
	sb_info_t *si;
	uint32 in, out, outen;
	...
	if (on) {
		...
		if (what & PLL)
			out |= PCI_CFG_GPIO_PLL;
		OSL_PCI_WRITE_CONFIG(si->osh, PCI_GPIO_OUT, sizeof(uint32),
				     out);
		OSL_PCI_WRITE_CONFIG(si->osh, PCI_GPIO_OUTEN,
				     sizeof(uint32), outen);
	}
...
bool sb_pwrctl_clk(void *sbh, uint mode)
{
	sb_info_t *si;
	uint origidx;
	chipcregs_t *cc;
	...
	case CLK_FAST:		/* force fast (pll) clock */
		...
		SET_REG(&cc->slow_clk_ctl, (SCC_XC | SCC_FS | SCC_IP),
			SCC_IP);
		break;
	...
	forcefastclk =
	    (bool) ((R_REG(&cc->slow_clk_ctl) & SCC_IP) == SCC_IP);
...
void
sb_register_intr_callback(void *sbh, void *intrsoff_fn,
			  void *intrsrestore_fn, void *intr_arg)
{
	sb_info_t *si;
