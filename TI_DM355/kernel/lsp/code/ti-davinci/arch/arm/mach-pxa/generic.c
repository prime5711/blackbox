...
void pxa_set_cken(int clock, int enable)
{
	...
	if (clock < 32) {
		if (enable) {
			/* GCU and USB2 are dependent on HSIO2 */
			if ((CKEN_GRAPHICS == clock) || (CKEN_USB2 == clock)) {
			...
			CKENA &= ~(0x1u << clock);
			if (CKEN_GRAPHICS == clock) {
				if ((CKENA & (1 << CKEN_USB2)) == 0) {
					CKENB &= ~(1 << (CKEN_HSIO2 - 32));
				}
			}
			if (CKEN_USB2 == clock) {
				if ((CKENA & (1 << CKEN_GRAPHICS)) == 0) {
					CKENB &= ~(1 << (CKEN_HSIO2 - 32));
				}
			}
		}
