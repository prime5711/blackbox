...
struct ipipe_def_cor_setup {
	enable_disable_t dfc_en;
	copy_method_t dfc_sel;
        unsigned int dfc_siz;
	unsigned int dfc_adr;
	unsigned int dfc_table[MAX_SIZE_DFC];
...
/* Programmable Noise Filter */
struct ipipe_prog_nf_setup {   
	enable_disable_t noise_fil_en;
        unsigned int d2f_cfg_spr;
	unsigned int d2f_cfg_shf;
	sampling_type_t type; 
        unsigned int d2f_thr[32];
        unsigned int d2f_str[32];
};
...
struct ipipe_rsz {
	...
	unsigned int rsz0_i_vst;
	unsigned int rsz0_i_hst;
	unsigned int rsz0_v_dif;
	unsigned int rsz0_h_dif;
	unsigned int rsz1_i_vst;
	unsigned int rsz1_i_hst;
	unsigned int rsz1_v_dif;
	unsigned int rsz1_h_dif;
};
...
/* RGB to RGB conversion (include GAMMA correction) */
struct ipipe_gamma_setup {
	gamma_cor_t gmm_cfg_bypr;
	gamma_cor_t gmm_cfg_bypg;
	gamma_cor_t gmm_cfg_bypb;
	gamma_tbl_t gmm_cfg_tbl;
	gamma_siz_t gmm_cfg_siz;
	unsigned short gmm_tbl_r[1024];
	unsigned short gmm_tbl_b[1024];
	unsigned short gmm_tbl_g[1024];
	unsigned short gmm_tbl_all[1024];
};
...
#define IPIPE_SET_WB	    	_IOWR(IPIPE_IOC_BASE,6, struct ipipe_wb)
#define IPIPE_SET_RSZ       	_IOWR(IPIPE_IOC_BASE,7, struct ipipe_rsz)
#define IPIPE_V_PAN         	_IOWR(IPIPE_IOC_BASE,8, int)
#define IPIPE_H_PAN         	_IOWR(IPIPE_IOC_BASE,9, int)
#define IPIPE_RGB2RGB         	_IOWR(IPIPE_IOC_BASE,10, struct ipipe_rgb2rgb)
#define IPIPE_BRT_CTR         	_IOWR(IPIPE_IOC_BASE,11, struct ipipe_rgb2yuv)
#define IPIPE_EDGE         	_IOWR(IPIPE_IOC_BASE,12, struct ipipe_edge_enhancer)
#define IPIPE_DFC         	_IOWR(IPIPE_IOC_BASE,13, struct ipipe_def_cor_setup)
#define IPIPE_IF_GAIN         	_IOWR(IPIPE_IOC_BASE,14, int)
#define IPIPE_GAMMA_SETUP      	_IOWR(IPIPE_IOC_BASE,15, struct ipipe_gamma_setup)
#define IPIPE_NF_SETUP      	_IOWR(IPIPE_IOC_BASE,16, struct ipipe_prog_nf_setup)
#define IPIPE_RGB2YUV      	_IOWR(IPIPE_IOC_BASE,17, struct ipipe_rgb2yuv)
#define IPIPE_FC_SUP      	_IOWR(IPIPE_IOC_BASE,18, struct ipipe_false_color_suppresion)
#define IPIPE_MIRROR      	_IOWR(IPIPE_IOC_BASE,19, int)
/*
*/
#define IPIPE_IOC_MAXNR 	(19)
/* End of ioctls */
