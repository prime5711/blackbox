...
#define IRQ_IPIPE_SDR		5/*request_irq*/
...
/* Resize */
#define RSZ0_I_VST		0x02CC
#define RSZ0_I_HST		0x02D4
#define RSZ0_V_DIF		0x02EC
#define RSZ0_H_DIF		0x02F8
#define RSZ1_I_VST		0x033C
#define RSZ1_I_HST		0x0344
#define RSZ1_V_DIF		0x035C
#define RSZ1_H_DIF		0x0368
...
static inline int ipipe_change_rsz1_setting( int width, int height, int diff )
{
	regw_ip((height - 1), RSZ_EN_1 + RSZ_O_VSZ);
        regw_ip((width - 1), RSZ_EN_1 + RSZ_O_HSZ);

	regw_ip(diff, RSZ_EN_1 + RSZ_V_DIF);
	regw_ip(diff, RSZ_EN_1 + RSZ_H_DIF);

	regw_ip(width<<1, RSZ_EN_1 + RSZ_SDR_OFT);
	regw_ip(height, RSZ_EN_1 + RSZ_SDR_PTR_E);

        return 0;
}


/* resize enable function */
