...
Kernel_image_file='/mnt/mmc/update/uImage_NC1000S_2.0M_2.0M_2012_1108_1726_rev238'
Rootfs_image_file='/mnt/mmc/update/nc1000s_fs_2013_0730_K9F1G08.bin.gz'
PC_Viwer_file='/mnt/mmc/update/NaviCAM.exe_'
Config_file='/mnt/mmc/update/nc100_blackbox.conf'
Script_file='/mnt/mmc/update/dev_postscript.sh.bak'


cp /mnt/mmc/update/fw_env.config_K9F1G08 /etc/fw_env.config
...
InitSystem()
{
    ...
    if [ -f $PC_Viwer_file ]
    then
        cp  $PC_Viwer_file /mnt/nand/pc_viewer/NaviCAM.exe
        sync
    else
	...
	then 
		...
		if [ -f $SvnRevision ]
			then 
				...
				if [ $CUR_SVN_REVISION != "NC1000S_208" ] 
				then 
